...
export default {
  ...
  mounted () {
    ...
    window.electronAPI.on('win.restore', this._restored)
    window.electronAPI.on('win.show', this._restored)
    window.electronAPI.on('win.focus', this._restored)
    window.electronAPI.on('win.minimize', this._minimized)
    window.electronAPI.on('win.hide', this._minimized)
    window.electronAPI.on('win.blur', this._minimized)
  },

  beforeDestroy () {
    window.removeEventListener('keydown', this.onKeyDown)
    window.electronAPI.off('win.restore', this._restored)
    window.electronAPI.off('win.show', this._restored)
    window.electronAPI.off('win.focus', this._restored)
    window.electronAPI.off('win.minimize', this._minimized)
    window.electronAPI.off('win.hide', this._minimized)
    window.electronAPI.off('win.blur', this._minimized)
    this.clearProgress()
  },
  ...
  methods: {
    ...
    async onPlayerActivated () {
      ...
      const visible = await window.electronAPI.invoke('win.isVisible')
      if (!visible) {
        this.setupProgress()
      }
    },
    ...
    setupProgress () {
      window.electronAPI.invoke('win.setProgressBar', [1, {
        mode: 'indeterminate'
      }])
    },
