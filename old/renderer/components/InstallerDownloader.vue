...
export default {
  ...
  methods: {
    updateApp () {
      ...
      this.updating = true
      window.electronAPI.send('do-update')
    },
    ...
    startDownload () {
      ...
      window.electronAPI.send('installer.download-file', payload)
    },
    runInstaller () {
      if (this.downloadedFilePath) {
        console.log('[InstallerDownloader] Running installer:', this.downloadedFilePath)
        this.installing = true
        window.electronAPI.send('installer.run', this.downloadedFilePath)
      } else {
  ...
  mounted () {
    ...
    window.electronAPI.on('download-progress', (event, data) => {
    ...
    window.electronAPI.on('download-complete', (event, data) => {
    ...
    window.electronAPI.on('download-error', (event, data) => {
  ...
  beforeDestroy () {
    ...
    window.electronAPI.removeAllListeners('installer.error')
  }
}
</script>
