  ...
  if (settings && settings.devMode) {
    ...
    template.push({
      ...
      submenu: [
        ...
        { id: 'save-for-test-runner', label: 'Save Test Scenario', click () { win.webContents.send('menu.save-for-test-runner') } },
        { type: 'separator' },
...
export default function () {
  ...
  ipcMain.handle('translate-menu', (ev, messages) => {
    if (_win) {
      ...
      Object.entries(enabledState).forEach(([id, enabled]) => {
        const item = m && m.getMenuItemById(id)
        if (item) item.enabled = enabled
      })
    }
  })
  ...
  return {
    winCreated (win) { _win = win; createMenu(win, {}) },
